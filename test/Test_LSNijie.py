...
from MediaDownloader import LSNijie
...
class TestLSNijie(unittest.TestCase):
    ...
    def __MakeLoginMock(self, mock: MagicMock) -> MagicMock:
        ...
        Note:
            ID/PWが一致すればOKとする
            対象のmockは "MediaDownloader.LSNijie.LSNijie.Login" にpatchする
    ...
    def test_LSNijie(self):
        ...
        with ExitStack() as stack:
            mocknslogin = stack.enter_context(patch("MediaDownloader.LSNijie.LSNijie.Login"))
            ...
            self.assertIsNone(lsn_cont.cookies)
            self.assertFalse(lsn_cont.auth_success)
    ...
    def test_Login(self):
        ...
        with ExitStack() as stack:
            ...
            # モック置き換え
            mocknsreqget = stack.enter_context(patch("MediaDownloader.LSNijie.requests.get"))
            mocknsreqpost = stack.enter_context(patch("MediaDownloader.LSNijie.requests.post"))
            mocknsreqcj = stack.enter_context(patch("MediaDownloader.LSNijie.requests.cookies.RequestsCookieJar"))
            mocknsisvalidcookies = stack.enter_context(patch("MediaDownloader.LSNijie.LSNijie.IsValidCookies"))
    ...
    def test_DownloadIllusts(self):
        ...
        with ExitStack() as stack:
            # モック置き換え
            mocknsreqget = stack.enter_context(patch("MediaDownloader.LSNijie.requests.get"))
            mocknsbs = stack.enter_context(patch("MediaDownloader.LSNijie.BeautifulSoup"))
            mocknsdpa = stack.enter_context(patch("MediaDownloader.LSNijie.LSNijie.DetailPageAnalysis"))
            mocknsmsdp = stack.enter_context(patch("MediaDownloader.LSNijie.LSNijie.MakeSaveDirectoryPath"))
