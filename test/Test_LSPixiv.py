...
from MediaDownloader import LSPixiv
...
class TestLSPixiv(unittest.TestCase):
    ...
    def __MakeLoginMock(self, mock: MagicMock) -> MagicMock:
        ...
        Note:
            ID/PWが一致すればOKとする
            対象のmockは "MediaDownloader.LSPixiv.LSPixiv.Login" にpatchする
    ...
    def test_LSPixiv(self):
        ...
        with ExitStack() as stack:
            mockpalogin = stack.enter_context(patch("MediaDownloader.LSPixiv.LSPixiv.Login"))
            ...
            self.assertIsNone(lsp_cont.api)
            self.assertIsNone(lsp_cont.aapi)
            self.assertFalse(lsp_cont.auth_success)
    ...
    def test_Login(self):
        ...
        with ExitStack() as stack:
            ...
            # mockpalogin = stack.enter_context(patch("MediaDownloader.LSPixiv.LSPixiv.Login"))
            mockpapub = stack.enter_context(patch("MediaDownloader.LSPixiv.PixivAPI"))
            mockpaapp = stack.enter_context(patch("MediaDownloader.LSPixiv.AppPixivAPI"))
    ...
    def test_DownloadIllusts(self):
        ...
        """
        with ExitStack() as stack:
            mockgu = stack.enter_context(patch("MediaDownloader.LSPixiv.LSPixiv.DownloadUgoira"))
            mocksleep = stack.enter_context(patch("MediaDownloader.LSPixiv.sleep"))
            mockpalogin = stack.enter_context(patch("MediaDownloader.LSPixiv.LSPixiv.Login"))
            mockpalogin = self.__MakeLoginMock(mockpalogin)
            lsp_cont = LSPixiv.LSPixiv(self.username, self.password, self.TEST_BASE_PATH)
    ...
    def test_DownloadUgoira(self):
        ...
        with ExitStack() as stack:
            mocksleep = stack.enter_context(patch("MediaDownloader.LSPixiv.sleep"))
            mockpalogin = stack.enter_context(patch("MediaDownloader.LSPixiv.LSPixiv.Login"))
            mockimage = stack.enter_context(patch("MediaDownloader.LSPixiv.Image"))
