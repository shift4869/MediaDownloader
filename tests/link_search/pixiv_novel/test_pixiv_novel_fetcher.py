...
logger = getLogger("media_downloader.link_search.pixiv_novel.pixiv_novel_fetcher")
    ...
    def get_instance(self) -> PixivNovelFetcher:
        with ExitStack() as stack:
            self.mock_login = stack.enter_context(
                patch("media_downloader.link_search.pixiv_novel.pixiv_novel_fetcher.PixivNovelFetcher.login")
            )
    ...
    def test_login(self):
        with ExitStack() as stack:
            mock_aapi = stack.enter_context(
                patch("media_downloader.link_search.pixiv_novel.pixiv_novel_fetcher.AppPixivAPI")
            )
    ...
    def test_fetch(self):
        with ExitStack() as stack:
            mock_save_directory_path = stack.enter_context(
                patch(
                    "media_downloader.link_search.pixiv_novel.pixiv_novel_fetcher.PixivNovelSaveDirectoryPath.create"
                )
            )
            mock_downloader = stack.enter_context(
                patch("media_downloader.link_search.pixiv_novel.pixiv_novel_fetcher.PixivNovelDownloader")
            )
